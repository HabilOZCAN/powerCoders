...
let newCalori;
let newTime;
let parentDiv = document.getElementById('productlist');

for (index = 0; index < productList.length; index++) {  
    newName = productList[`${index}`].productName;
    newTime = productList[`${index}`].expireDate;
    newPrice = productList[`${index}`].price;
    newCalori = productList[`${index}`].totalCalories;
    newImageSrc = productList[`${index}`].productImage
    parentDiv.innerHTML += `<div class = "generated">
                                <p>${newName}</p>
                                <p>${newTime.getDate()}-${newTime.getMonth()+1}-${newTime.getFullYear()}</p>
                                <p>${newCalori}</p>
                                <p>${newPrice} CH</p>
                                <img src="${newImageSrc}">
